...
//		add_theme_support( 'title-tag' );
		...
		register_nav_menus(
			array(
				'header_menu' => esc_html__( 'Header Menu', 'flowers' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'flowers_custom_background_args',
				array(
...
function flowers_widgets_init() {
    ...
    register_sidebar(
        array(
            'name'          => esc_html__( 'Footer Right', 'flowers' ),
            'id'            => 'sidebar-footer1',
            'description'   => esc_html__( 'Add widgets here.', 'flowers' ),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
...
//Custom code
function flowers_debug($data)
{
    echo '<pre>'. print_r($data, 1) . '</pre>';
}

function flowers_get_background($field, $cat = null, $cover = true)
{
...
function flowers_get_background_no_center($field, $cat = null, $cover = true)
{
    if (get_field($field, $cat))
    {
        $add_style = $cover ? 'background-size: cover' : '';
...
function flowers_comment( $comment, $args, $depth ) {
    if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
    ...
    <div class="comment-author vcard">
        ...
        printf(
            __( '<b class="fn fn-padding">%s</b> <span class="comment-date">'.get_comment_date().'NTCN</span>' ),
